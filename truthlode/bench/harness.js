// Truthlode Bench — harness. Runs a set of agent "submissions" through the verifier
...
//
// Usage: node truthlode/bench/harness.js <submissions.json> [outdir]
...
const CONCURRENCY = parseInt(process.env.TRUTHLODE_CONCURRENCY || '4', 10)
...
async function main () {
  const [,, submissionsPath, outDir = 'truthlode/bench/out'] = process.argv
  if (!submissionsPath) { console.error('usage: node truthlode/bench/harness.js <submissions.json> [outdir]'); process.exit(1) }
  const raw = JSON.parse(await fs.readFile(submissionsPath, 'utf-8'))
  const agents = Array.isArray(raw) ? raw : [raw]
  await fs.mkdir(outDir, { recursive: true })
  process.stderr.write(`[truthlode] judge: ${judgeLabel()}\n`)

  const board = []; const audit = []
  for (const agent of agents) {
    const name = agent.agent || 'unknown'
    process.stderr.write(`[truthlode] ${name}: ${agent.citations.length} citations...\n`)
    ...
    process.stderr.write(`[truthlode] ${name}: support ${(score.supportRate * 100).toFixed(1)}% | fabricated ${(score.fabricatedRate * 100).toFixed(1)}% | ${score.scorable}/${score.total} scorable\n`)
  }
  board.sort(cmpRank)
  ...
  process.stderr.write(`[truthlode] wrote ${outDir}/leaderboard.md + audit.json\n`)
