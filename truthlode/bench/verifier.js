// Truthlode Bench — citation→claim verifier (v2).
...
//   Override with TRUTHLODE_JUDGE_MODEL / TRUTHLODE_JUDGE_PROVIDER.
...
const JUDGE_MODEL = process.env.TRUTHLODE_JUDGE_MODEL || 'qwen2.5:14b-instruct'
const JUDGE_TIMEOUT_MS = parseInt(process.env.TRUTHLODE_JUDGE_TIMEOUT_MS || '120000', 10)
...
// cached verdicts. For a fresh deterministic re-judge, pin a dated snapshot via TRUTHLODE_DEEPSEEK_MODEL
// or use the local qwen2.5:14b-instruct path (TRUTHLODE_JUDGE_PROVIDER=ollama).
const DEEPSEEK_MODEL = process.env.TRUTHLODE_DEEPSEEK_MODEL || 'deepseek-chat'

// Which judge will run (evaluated lazily so harness can load .env.local before first call).
export function judgeLabel () {
  const provider = process.env.TRUTHLODE_JUDGE_PROVIDER || (process.env.DEEPSEEK_API_KEY ? 'deepseek' : 'ollama')
...
const CACHE_DIR = process.env.TRUTHLODE_CACHE_DIR || 'truthlode/bench/.cache'
...
/**
 * Verify one citation (cached). Set TRUTHLODE_NO_CACHE=1 to force recompute.
 * @param {{ claim: string, url: string }} input
 */
export async function verifyCitation (input) {
  if (process.env.TRUTHLODE_NO_CACHE) return computeVerdict(input)
