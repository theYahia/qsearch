...
//   truthlode.csv  flat per-agent metrics (the "download the data" artifact)
...
// Usage: node truthlode/site/build_site.mjs   (run from anywhere; paths are relative to this file)
import fs from 'node:fs'

const SITE_URL = 'https://truthlode.org'
const REPO_URL = 'https://github.com/theYahia/truthlode'
...
// ── truthlode.csv — flat per-agent metrics ──
...
fs.writeFileSync(new URL('./truthlode.csv', import.meta.url), csv)
...
<div class="kicker"><span class="kb">Truthlode</span> · the citation-honesty record</div>
...
const titleText = `Truthlode — ${phrase} checkable AI citations doesn't fully hold up`
const ogTitle = `${phrase} checkable AI citations doesn't fully hold up — Truthlode`
...
const jsonld = {
  ...
  '@type': 'Dataset',
  name: 'Truthlode — citation-honesty audit of AI deep-research agents',
  description: desc,
  url: SITE_URL,
  sameAs: REPO_URL,
  ...
  distribution: [
    ...
    { '@type': 'DataDownload', encodingFormat: 'text/csv', contentUrl: `${SITE_URL}/truthlode.csv` }
  ]
}
